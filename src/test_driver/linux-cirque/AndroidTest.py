...
class TestAndroidController(CHIPVirtualHome):
    ...
    def run_controller_test(self):
        self.android_emulator_device_id = [device["id"] for device in self.non_ap_devices
                                   if device["type"] == "ANDROID-EMULATOR"][0]
        ...
        self.execute_device_cmd(self.android_emulator_device_id, "adb kill-server")
    ...
    def save_device_logs(self):
        ...
        if copy_report_ret != 0:
            print("Failed to copy Gradle test reports: " + str(copy_report_ret))
...
def _add_emulator_version_tag(config):
    version_path = os.path.join(CHIP_REPO, "integrations/docker/ci-only-images/android-emulator/version")
    version = subprocess.check_output("sed 's/ .*//' " + version_path, shell=True).decode("utf-8")
    # for device in config.values():
    #     if device["base_image"] == "connectedhomeip/android-emulator":
    #         device["base_image"] = "connectedhomeip/android-emulator:" + version
    return config

DEVICE_CONFIG = {}
with open(os.path.join(CHIP_REPO, "src/test_driver/linux-cirque/topologies/one_node_one_android.json"), "r") as f:
    config_operations = [_parse_mount_dir, _add_emulator_version_tag]
