...
DEVICE_CONFIG = {
    'device0': {
        ...
        'capability': ['Interactive', 'Mount', "Bluetooth"],
        'traffic_control': {'latencyMs': 100},
        'entrypoint': ['/bin/CHIPCirqueDaemon.py', '--server'],
...
class TestPythonController(CHIPVirtualHome):
    ...
    def run_controller_test(self):
        ...
        for server in server_ids:
            self.execute_device_cmd(server, "CHIPCirqueDaemon.py -- run {}".format(
                os.path.join(CHIP_REPO, "out/debug/standalone/chip-all-clusters-app --ble-device 0")))
        ...
        # We only enable data model tests on BLE device since we cannot enable thread when bluetooth is enabled.
        command = "gdb -return-child-result -q -ex run -ex bt --args python3 {} -t 150 --commissioning ble --disable-test nwprov --bluetooth-adapter 1".format(
            os.path.join(
