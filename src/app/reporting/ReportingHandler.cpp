...
CHIP_ERROR SubscriptionEngine::Init(chip::WeaveExchangeManager * const apExchangeMgr, void * const aAppState,
                                     const EventCallback aEventCallback)
{
    ...
    err = mExchangeMgr->RegisterUnsolicitedMessageHandler(chip::Profiles::kWeaveProfile_WDM, UnsolicitedMessageHandler, this);
    SuccessOrExit(err);
...
void SubscriptionEngine::OnCancelRequest(chip::ExchangeContext * aEC, const nl::Inet::IPPacketInfo * aPktInfo,
                                         ...
                                         PacketBuffer * aPayload)
{
    ...
    if (!found)
    {
        err = SendStatusReport(aEC, chip::Profiles::kWeaveProfile_WDM, kStatus_InvalidSubscriptionID);
        SuccessOrExit(err);
    }
...
CHIP_ERROR SubscriptionEngine::NewClient(SubscriptionClient ** const appClient, Binding * const apBinding, void * const apAppState,
 ...
 */
CHIP_ERROR SubscriptionEngine::SendStatusReport(chip::ExchangeContext * aEC, uint32_t aProfileId, uint16_t aStatusCode)
{
    CHIP_ERROR err = WEAVE_NO_ERROR;
 ...
 */
void SubscriptionEngine::UnsolicitedMessageHandler(chip::ExchangeContext * aEC, const nl::Inet::IPPacketInfo * aPktInfo,
 ...
 * This function is a @ref ExchangeContext::MessageReceiveFunct.
 */
void SubscriptionEngine::OnUnknownMsgType(chip::ExchangeContext * aEC, const nl::Inet::IPPacketInfo * aPktInfo,
                                          const chip::WeaveMessageInfo * aMsgInfo, uint32_t aProfileId, uint8_t aMsgType,
                                          PacketBuffer * aPayload)
{
    CHIP_ERROR err = WEAVE_NO_ERROR;
...
void SubscriptionEngine::OnNotificationRequest(chip::ExchangeContext * aEC, const nl::Inet::IPPacketInfo * aPktInfo,
                                               const chip::WeaveMessageInfo * aMsgInfo, uint32_t aProfileId, uint8_t aMsgType,
                                               PacketBuffer * aPayload)
{
    CHIP_ERROR err                    = WEAVE_NO_ERROR;
...
CHIP_ERROR SubscriptionEngine::ProcessDataList(chip::TLV::TLVReader & aReader,
...
#if WDM_ENABLE_SUBSCRIPTIONLESS_NOTIFICATION
CHIP_ERROR SubscriptionEngine::RegisterForSubscriptionlessNotifications(const TraitCatalogBase<TraitDataSink> * const apCatalog)
{
    CHIP_ERROR err                    = WEAVE_NO_ERROR;
...
void SubscriptionEngine::OnSubscriptionlessNotification(chip::ExchangeContext * aEC, const nl::Inet::IPPacketInfo * aPktInfo,
                                                        const chip::WeaveMessageInfo * aMsgInfo, uint32_t aProfileId,
                                                        uint8_t aMsgType, PacketBuffer * aPayload)
{
    CHIP_ERROR err = WEAVE_NO_ERROR;
    ...
    if (isDataListPresent)
    {
        ...
        if (isPartialChange)
        {
            ...
            // data info.
            ExitNow(err = CHIP_ERROR_WDM_SUBSCRIPTIONLESS_NOTIFY_PARTIAL);
        }
    }
...
CHIP_ERROR SubscriptionEngine::SubscriptionlessNotifyDataElementAccessControlDelegate::DataElementAccessCheck(
    const TraitPath & aTraitPath, const TraitCatalogBase<TraitDataSink> & aCatalog)
{
    ...
    else
    {
        ...
        outParam.mDataElementAccessControlForNotification.mReason             = CHIP_ERROR_ACCESS_DENIED;
    }
...
CHIP_ERROR SubscriptionEngine::GetMinEventLogPosition(size_t & outLogPosition) const
{
    CHIP_ERROR err = WEAVE_NO_ERROR;
...
CHIP_ERROR SubscriptionEngine::EnablePublisher(IWeavePublisherLock * aLock,
...
CHIP_ERROR SubscriptionEngine::Lock()
{
    if (mLock)
    {
        return mLock->Lock();
...
CHIP_ERROR SubscriptionEngine::Unlock()
{
    if (mLock)
    {
        return mLock->Unlock();
...
CHIP_ERROR SubscriptionEngine::NewSubscriptionHandler(SubscriptionHandler ** subHandler)
{
    CHIP_ERROR err = CHIP_ERROR_NO_MEMORY;
...
void SubscriptionEngine::OnSubscribeRequest(chip::ExchangeContext * aEC, const nl::Inet::IPPacketInfo * aPktInfo,
                                            const chip::WeaveMessageInfo * aMsgInfo, uint32_t aProfileId, uint8_t aMsgType,
                                            PacketBuffer * aPayload)
{
    CHIP_ERROR err                    = WEAVE_NO_ERROR;
...
void SubscriptionEngine::OnSubscribeConfirmRequest(chip::ExchangeContext * aEC, const nl::Inet::IPPacketInfo * aPktInfo,
                                                   const chip::WeaveMessageInfo * aMsgInfo, uint32_t aProfileId,
                                                   uint8_t aMsgType, PacketBuffer * aPayload)
{
    CHIP_ERROR err                    = WEAVE_NO_ERROR;
...
void SubscriptionEngine::OnCustomCommand(chip::ExchangeContext * aEC, const nl::Inet::IPPacketInfo * aPktInfo,
                                         const chip::WeaveMessageInfo * aMsgInfo, uint32_t aProfileId, uint8_t aMsgType,
                                         PacketBuffer * aPayload)
{
    CHIP_ERROR err                    = WEAVE_NO_ERROR;
    ...
    uint32_t statusReportProfile       = chip::Profiles::kWeaveProfile_WDM;
    uint16_t statusReportCode          = chip::Profiles::DataManagement::kStatus_InvalidPath;

    for (size_t i = 0; i < kMaxNumCommandObjs; ++i)
    {
        if (pEngine->mCommandObjs[i].IsFree())
        {
            SYSTEM_STATS_INCREMENT(chip::System::Stats::kWDM_NumCommands);
    ...
    VerifyOrExit(NULL != command, err = CHIP_ERROR_NO_MEMORY);

    if (!pEngine->mIsPublisherEnabled)
    {
        // Has to be a publisher to be processing a command
        statusReportProfile = chip::Profiles::kWeaveProfile_Common;
        statusReportCode    = chip::Profiles::Common::kStatus_UnsupportedMessage;
...

    {
...

        {
            ...
            if (!dataSource->GetSchemaEngine()->GetVersionIntersection(requestedSchemaVersion, computedVersionIntersection))
            {
                ...
                statusReportProfile = chip::Profiles::kWeaveProfile_WDM;
                statusReportCode    = kStatus_IncompatibleDataSchemaVersion;
                ExitNow(err = CHIP_ERROR_INCOMPATIBLE_SCHEMA_VERSION);
            }
        ...
        if (command->IsExpiryTimeValid())
        {
            ...
            {
                statusReportCode = chip::Profiles::DataManagement::kStatus_ExpiryTimeNotSupported;
                ExitNow();
            }
            else if (WEAVE_SYSTEM_ERROR_REAL_TIME_NOT_SYNCED == err)
            {
                statusReportCode = chip::Profiles::DataManagement::kStatus_NotTimeSyncedYet;
                ExitNow();
            }
            else if (now_usec >= (uint64_t) command->expiryTimeMicroSecond)
            {
                statusReportCode = chip::Profiles::DataManagement::kStatus_RequestExpiredInTime;
                ExitNow();
            }
        ...
        if (command->IsMustBeVersionValid())
        {
            ...
            if (command->mustBeVersion != currentVersion)
            {
                WeaveLogDetail(DataManagement, "Version required 0x%" PRIX64 ", current: 0x% " PRIX64 " ", command->mustBeVersion,
                               currentVersion);
                statusReportCode = chip::Profiles::DataManagement::kStatus_VersionMismatch;
                ExitNow();
            }
        }
...
#if WDM_ENABLE_PUBLISHER_UPDATE_SERVER_SUPPORT
CHIP_ERROR SubscriptionEngine::AllocateRightSizedBuffer(PacketBuffer *& buf, const uint32_t desiredSize, const uint32_t minSize,
 ...
 */
CHIP_ERROR SubscriptionEngine::InitializeStatusDataHandleList(Weave::TLV::TLVReader & aReader,
                                                               ...
                                                               uint32_t & aNumDataElements, uint8_t * apBufEndAddr)
{
    ...
    for (aNumDataElements = 0; WEAVE_NO_ERROR == (err = dataReader.Next()); aNumDataElements++)
    {
        ...
        VerifyOrExit((uint8_t *) (apStatusDataHandleList + aNumDataElements + 1) <= apBufEndAddr, err = CHIP_ERROR_NO_MEMORY);
...
void SubscriptionEngine::ConstructStatusListVersionList(chip::TLV::TLVWriter & aWriter, void * apContext)
{
    CHIP_ERROR err                                 = WEAVE_NO_ERROR;
    chip::TLV::TLVWriter checkpoint            = aWriter;
    TraitDataSource * dataSource                    = NULL;
    ...
    VerifyOrExit(NULL != apContext, err = CHIP_ERROR_INCORRECT_STATE);
...
void SubscriptionEngine::UpdateStatusDataHandleElement(StatusDataHandleElement * apStatusDataHandleList,
                                                       TraitDataHandle aTraitDataHandle, CHIP_ERROR & err, uint32_t aCurrentIndex)
{
    uint32_t profileId  = 0;
    uint16_t statusCode = 0;

    if (CHIP_ERROR_ACCESS_DENIED == err)
    {
        profileId  = Weave::Profiles::kWeaveProfile_Common;
        statusCode = Weave::Profiles::Common::kStatus_AccessDenied;
        err        = WEAVE_NO_ERROR;
    }
    else if (CHIP_ERROR_INVALID_PROFILE_ID == err)
    {
        profileId  = Weave::Profiles::kWeaveProfile_WDM;
        statusCode = kStatus_InvalidPath;
        err        = WEAVE_NO_ERROR;
    }
    else if (CHIP_ERROR_WDM_VERSION_MISMATCH == err)
    {
        profileId  = Weave::Profiles::kWeaveProfile_WDM;
        statusCode = kStatus_VersionMismatch;
        err        = WEAVE_NO_ERROR;
    }
    else if (CHIP_ERROR_WRONG_TLV_TYPE == err || CHIP_ERROR_TLV_TAG_NOT_FOUND == err)
    {
        profileId  = Weave::Profiles::kWeaveProfile_WDM;
                                        ...
                                        uint32_t aCurrentIndex)
{
    ...
    {
        if ((chip::Profiles::kWeaveProfile_Common == apStatusDataHandleList[index].mProfileId) &&
            (chip::Profiles::Common::kStatus_Success == apStatusDataHandleList[index].mStatusCode) &&
 ...
 * for each trait
 */
CHIP_ERROR SubscriptionEngine::UpdateTraitVersions(StatusDataHandleElement * apStatusDataHandleList,
 ...
 */
CHIP_ERROR SubscriptionEngine::SendFaultyUpdateResponse(Weave::ExchangeContext * apEC)
{
    CHIP_ERROR err = WEAVE_NO_ERROR;
    uint8_t * p;
    ...
    p = msgBuf->Start();
    chip::Encoding::LittleEndian::Write32(p, Weave::Profiles::kWeaveProfile_Common);
    chip::Encoding::LittleEndian::Write16(p, Weave::Profiles::Common::kStatus_BadRequest);
 ...
 */
CHIP_ERROR SubscriptionEngine::SendUpdateResponse(Weave::ExchangeContext * apEC, uint32_t aNumDataElements,
 ...
 */
CHIP_ERROR SubscriptionEngine::ProcessUpdateRequestDataElement(Weave::TLV::TLVReader & aReader, TraitDataHandle & aHandle,
                                                                ...
                                                                StatusDataHandleElement * apStatusDataHandleList)
{
    ...
    VerifyOrExit(isPartialChange == false, err = CHIP_ERROR_INCORRECT_STATE);
    ...
    if (aConditionalLoop)
    {
        ...
        VerifyOrExit(versionInTrait == requiredVersion, err = CHIP_ERROR_WDM_VERSION_MISMATCH);
    }
    else
    {
        VerifyOrExit(!isConditionalDE, needSkip = true);
 ...
 */
CHIP_ERROR SubscriptionEngine::ProcessUpdateRequestDataListWithConditionality(
    ...
    bool & aExistFailure, bool aConditionalLoop)
{
    ...
    for (uint32_t index = 0; WEAVE_NO_ERROR == (err = dataReader.Next()); index++)
    {
        TraitDataHandle handle;
        PropertyPathHandle pathHandle;
        if (!(apStatusDataHandleList[index].mProfileId == chip::Profiles::kWeaveProfile_Common && apStatusDataHandleList[index].mStatusCode == chip::Profiles::Common::kStatus_Success))
        {
 ...
 */
CHIP_ERROR SubscriptionEngine::ProcessUpdateRequestDataList(Weave::TLV::TLVReader & aReader,
                                                             StatusDataHandleElement * apStatusDataHandleList,
                                                             const TraitCatalogBase<TraitDataSource> * apCatalog,
                                                             IUpdateRequestDataElementAccessControlDelegate & acDelegate,
 ...
 */
CHIP_ERROR SubscriptionEngine::ProcessUpdateRequest(Weave::ExchangeContext * apEC, Weave::TLV::TLVReader & aReader,
                                                     const TraitCatalogBase<TraitDataSource> * apCatalog,
                                                     IUpdateRequestDataElementAccessControlDelegate & acDelegate)
{
    CHIP_ERROR err                                = WEAVE_NO_ERROR;
    ...
    VerifyOrExit(apCatalog != NULL, err = CHIP_ERROR_INVALID_ARGUMENT);
...
CHIP_ERROR SubscriptionEngine::UpdateRequestDataElementAccessControlDelegate::DataElementAccessCheck(
    const TraitPath & aTraitPath, const TraitCatalogBase<TraitDataSource> & aCatalog)
{
    ...
    else
    {
        ...
        outParam.mDataElementAccessControlForUpdateRequest.mReason              = CHIP_ERROR_ACCESS_DENIED;
    }
