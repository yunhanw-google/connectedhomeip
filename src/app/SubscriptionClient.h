...
public:
    ...
    enum EventID
    {
         ...
         * ::chip::Profiles::DataManagement_Current::SubscriptionClient::EventCallback
         ...
         * |Error code | Meaning |
         * |-----------|---------|
         * | #CHIP_ERROR_INVALID_MESSAGE_TYPE | An unrecognized message is received. |
         * | #CHIP_ERROR_TIMEOUT | An ack is not received or a liveness check fails.|
         * | #CHIP_ERROR_INCORRECT_STATE | Messages are received in an unexpected state. |
         * | #CHIP_ERROR_STATUS_REPORT_RECEIVED | A status report is received. |
         * | #CHIP_ERROR_INVALID_ARGUMENT | Subscribe request fields are invalid. |
         */
    ...
    struct ResubscribeParam
    {
        ...
        CHIP_ERROR mReason;      ///< Error received on most recent failure
     ...
     */
    CHIP_ERROR EndSubscription(void);
    ...
    CHIP_ERROR GetSubscriptionId(uint64_t * const apSubscriptionId);
    ...
    CHIP_ERROR FlushUpdate();
    CHIP_ERROR FlushUpdate(bool aForce);
    CHIP_ERROR SetUpdated(TraitUpdatableDataSink * aDataSink, PropertyPathHandle aPropertyHandle, bool aIsConditional);
    CHIP_ERROR ClearUpdated(TraitUpdatableDataSink * aDataSink, PropertyPathHandle aPropertyHandle);
    void DiscardUpdates();
...
private:
    ...
    // null out EC
    CHIP_ERROR Init(Binding * const apBinding, void * const apAppState, EventCallback const aEventCallback,
    ...
    CHIP_ERROR SendSubscribeRequest(void);

    void _Cleanup();

    CHIP_ERROR ProcessDataList(chip::TLV::TLVReader & aReader);

    void _AddRef(void);
    void _Release(void);

    void TerminateSubscription(CHIP_ERROR aReason, Profiles::StatusReporting::StatusReport * aStatusReport, bool suppressAppCallback);
    CHIP_ERROR _PrepareBinding(void);

    CHIP_ERROR ReplaceExchangeContext(void);

    void FlushExistingExchangeContext(const bool aAbortNow = false);

    void NotificationRequestHandler(chip::ExchangeContext * aEC, const nl::Inet::IPPacketInfo * aPktInfo,
                                    const chip::WeaveMessageInfo * aMsgInfo, PacketBuffer * aPayload);

#if WDM_ENABLE_SUBSCRIPTION_CANCEL
    void CancelRequestHandler(chip::ExchangeContext * aEC, const nl::Inet::IPPacketInfo * aPktInfo,
                              const chip::WeaveMessageInfo * aMsgInfo, PacketBuffer * aPayload);
#endif // WDM_ENABLE_SUBSCRIPTION_CANCEL

    void TimerEventHandler(void);
    CHIP_ERROR RefreshTimer(void);

    void SetRetryTimer(CHIP_ERROR aReason); // TODO: rename to SetResubscribeTimer()
    ...
    static void OnSendError(ExchangeContext * aEC, CHIP_ERROR aErrorCode, void * aMsgSpecificContext);
    static void OnResponseTimeout(chip::ExchangeContext * aEC);
    static void OnMessageReceivedFromLocallyInitiatedExchange(chip::ExchangeContext * aEC,
    ...
    void StartUpdateRetryTimer(CHIP_ERROR aReason);
    ...
    CHIP_ERROR SendSingleUpdateRequest(void);
    static CHIP_ERROR AddElementFunc(UpdateEncoder * aEncoder, void * apCallState, TLV::TLVWriter & aOuterWriter);
    void SetUpdateStartVersions(void);

    // Methods to handle update response and exchange failures (OnResponseTimeout, OnSendError)
    void OnUpdateResponse(CHIP_ERROR aReason, chip::Profiles::StatusReporting::StatusReport * apStatus);
    void OnUpdateNoResponse(CHIP_ERROR aReason);
    static bool WillRetryUpdate(CHIP_ERROR aErr, uint32_t aStatusProfileId, uint16_t aStatusCode);

    // Methods to purge obsolete pending paths
    CHIP_ERROR PurgePendingUpdate(void);
    ...
    CHIP_ERROR PurgeFailedPendingPaths(CHIP_ERROR aErr, size_t & aCount);
    ...
    void PurgeAndNotifyFailedPaths(CHIP_ERROR aErr, TraitPathStore & aPathStore, size_t & aCount);
    ...
    CHIP_ERROR MovePendingToInProgress(void);
    CHIP_ERROR AddItemPendingUpdateSet(const TraitPath & aItem, const TraitSchemaEngine * const aSchemaEngine);
    CHIP_ERROR MoveInProgressToPending(void);
    ...
    void UpdateCompleteEventCbHelper(const TraitPath & aTraitPath, uint32_t aStatusProfileId, uint16_t aStatusCode,
                                     CHIP_ERROR aReason, bool aWillRetry);
