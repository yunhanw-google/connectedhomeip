...
public:
    ...
    CHIP_ERROR GetNodeId(uint64_t * const apNodeId) const;

    // CHIP_END_OF_TLV if there is no such element
    // CHIP_ERROR_WRONG_TLV_TYPE if there is such element but it's not any of the defined unsigned integer types
    CHIP_ERROR GetEndpointId(uint16_t * const apEndpointId) const;
...
public:
    ...
    AttributePath::Builder & NodeId(const uint64_t aNodeId);
    AttributePath::Builder & EndpointId(const uint16_t aEndpointId);
...
public:
    ...
    EventPath::Builder & EndpointId(const uint16_t aEndpointId);
 ...
 */
class CommandPath::Parser : public ParserBase
{
public:
    // aReader has to be on the element of EventPath container
    CHIP_ERROR Init(const chip::TLV::TLVReader & aReader);
    ...
    // CHIP_END_OF_TLV if there is no such element
    // CHIP_ERROR_WRONG_TLV_TYPE if there is such element but it's not any of the defined unsigned integer types
    CHIP_ERROR GetGroupId(uint32_t * const apGroupId) const;
    ...
    // CHIP_END_OF_TLV if there is no such element
    // CHIP_ERROR_WRONG_TLV_TYPE if there is such element but it's not any of the defined unsigned integer types
    CHIP_ERROR GetCommandId(uint16_t * const apEventId) const;
};

class CommandPath::Builder : public BuilderBase
{
public:
    CHIP_ERROR Init(chip::TLV::TLVWriter * const apWriter);
    CHIP_ERROR Init(chip::TLV::TLVWriter * const apWriter, const uint8_t aContextTagToUse);

    CommandPath::Builder & EndpointId(const uint16_t aEndpointId);
    CommandPath::Builder & GroupId(const uint32_t aGroupId);
    CommandPath::Builder & ClusterId(const uint32_t aClusterId);
    CommandPath::Builder & CommandId(const uint16_t aCommandId);

    CommandPath::Builder & EndOfCommandPath(void);
 ...
 *    Chip Interaction Model Data Element parser definition
 */
class CommandDataElement::Parser : public ParserBase
{
public:
    // aReader has to be on the element of DataElement
    CHIP_ERROR Init(const chip::TLV::TLVReader & aReader);
 ...
 */
class CommandDataElement::Builder : public BuilderBase
{
public:
    // DataElement is only used in a Data List, which requires every path to be anonymous
    CHIP_ERROR Init(chip::TLV::TLVWriter * const apWriter);

    CommandPath::Builder & CreateCommandPathBuilder(void);
    StatusCode::Builder & CreateStatusCodeBuilder(void);

    CommandDataElement::Builder & EndOfCommandDataElement(void);
...
class CommandList::Parser : public ListParserBase
{
public:
    // Roughly verify the schema is right, including
    // 1) at least one element is there
...
class CommandList::Builder : public ListBuilderBase
{
public:
    CommandDataElement::Builder & CreateCommandDataElementBuilder(void);
    CommandList::Builder & EndOfCommandList(void);
 ...
 */
class InvokeCommandRequest::Parser : public ParserBase
{
public:
    /**
 ...
 */
class InvokeCommandRequest::Builder : public BuilderBase
{
public:
    /**
     ...
     */
    CommandList::Builder & CreateCommandListBuilder(void);
     ...
     */
    InvokeCommandRequest::Builder & EndOfInvokeCommandRequest(void);
