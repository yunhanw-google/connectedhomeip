...
class BaseTestHelper:
    def __init__(self, nodeid: int, testCommissioner: bool = False, bluetoothAdapter: int = None):
        self.chipStack = ChipStack('/tmp/repl_storage.json')
        self.fabricAdmin = chip.FabricAdmin.FabricAdmin(
            fabricId=1, fabricIndex=1)
        self.devCtrl = self.fabricAdmin.NewController(nodeid, testCommissioner, bluetoothAdapter=bluetoothAdapter)
